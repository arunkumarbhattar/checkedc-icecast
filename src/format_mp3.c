...
int format_mp3_get_plugin (source_t *source : itype(_Ptr<struct source_tag>))
{
    ...
    metadata = (_Nt_array_ptr<char>) httpp_getvar (source->parser, "icy-metaint");
    if (metadata)
    {
...
static void mp3_set_tag (_Ptr<format_plugin_t> plugin, _Nt_array_ptr<const char> tag, _Nt_array_ptr<const char> in_value, _Nt_array_ptr<const char> charset)
{
    ...
    if (in_value)
    {
        value = (_Nt_array_ptr<char>)util_conv_string(in_value, charset, plugin->charset);
        if (value == NULL)
...
static void format_mp3_apply_settings (_Ptr<client_t> client, _Ptr<format_plugin_t> format, _Ptr<mount_proxy> mount)
{
    ...
    {
        _Nt_array_ptr<const char> metadata = (_Nt_array_ptr<char>)httpp_getvar (client->parser, "icy-metaint");
...
static int send_stream_metadata (_Ptr<client_t> client, _Ptr<refbuf_t> associated)
{
    int ret = 0;
    int meta_len = 0;
    _Array_ptr<char> metadata : count(meta_len) = ((void *)0);
    ...
    {
        metadata = associated->data + client_mp3->metadata_offset, meta_len = associated->len - client_mp3->metadata_offset;
    }
    else
    {
        if (associated)
        ...
        else
        {
            ...
            metadata = meta + client_mp3->metadata_offset, meta_len = 17 - client_mp3->metadata_offset;
        }
    }
        ...
        if (client_mp3->interval)
        {
            ...
            if (remaining <= len)
            {
                ...
                if (len > client_mp3->interval)
                    buf = _Dynamic_bounds_cast<_Array_ptr<char>>(buf, count(client_mp3->interval)), len = client_mp3->interval;
            }
        }
...
#define REFBUF_SIZE 1400
    ...
    int size = REFBUF_SIZE - source_mp3->read_count;
    _Array_ptr<char> buf : count(size) = source_mp3->read_data->data + source_mp3->read_count;
...
static _Ptr<refbuf_t> mp3_get_filter_meta(_Ptr<source_t> source)
{
    ...
    refbuf->data = _Dynamic_bounds_cast<_Nt_array_ptr<char>>(refbuf->data, count(0)), refbuf->len = 0;
    ...
    while (bytes > 0)
    {
	    ...
	    ICECAST_LOG_DEBUG("shoutcast metadata %.*s", 4080, meta->data+1);
            ...
            {
                filter_shoutcast_metadata (source, _Assume_bounds_cast<_Nt_array_ptr<char>>(source_mp3->build_metadata, byte_count(0)),
                        source_mp3->build_metadata_len);
                refbuf_release (source_mp3->metadata);
...
static int format_mp3_create_client_data(_Ptr<source_t> source, _Ptr<client_t> client)
{
    ...
    _Nt_array_ptr<char> ptr : count(remaining) = client->refbuf->data + client->refbuf->len - 2;
    int bytes;
    ...
    useragent = (_Nt_array_ptr<char>) httpp_getvar (client->parser, "user-agent");
    ...
    client->refbuf->data = _Dynamic_bounds_cast<_Nt_array_ptr<char>>( 
        client->refbuf->data, 
        count(4096 - remaining)),
